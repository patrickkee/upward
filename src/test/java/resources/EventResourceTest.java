...
public class EventResourceTest extends BaseJerseyTest {
	...
	public void updateEventTest() {
		...
		//Create a new modified event with the same id/name and post it
		Actual eventNew = Actual.getNew("Foo", startDate, BigDecimal.valueOf(200));
		response = target("accounts/" + EMAIL + "/models/" + MODEL_ID + "/events/" + String.valueOf(event.getEventId()))
				.request(MediaType.APPLICATION_JSON_TYPE)
				.post(Entity.entity(eventNew, MediaType.APPLICATION_JSON_TYPE), Response.class);
		
		ResponseMessage respMsg = response.readEntity(ResponseMessage.class);
		assertEquals(201, response.getStatus());
		assertTrue(response.getLocation().toString().contains("/accounts/createEventTest@gmail.com/models/" + 
															   MODEL_ID+"/events/" + Integer.toString(eventId)));
		assertEquals("EVENT_UPDATE_SUCCESSFUL", respMsg.getMessage());
		assertEquals("The event was successfully updated and persisted to the model", respMsg.getDescription());
	}
	...
	public void deleteEventTest() {
		final String EMAIL = "deleteEventTest@gmail.com";
		final String START_DATE = "01/25/2010";
		final String END_DATE = "01/25/2020";
		Response response = target("accounts").queryParam("accountName", "createModelTest")
				.queryParam("firstName", "createModelTestFirstName").queryParam("lastName", "createModelTestLastName")
		...
		//Delete the event
		response = target("accounts/" + EMAIL + "/models/" + MODEL_ID + "/events/" + String.valueOf(eventId))
					.request(MediaType.APPLICATION_JSON_TYPE)
					.delete(Response.class);	
	
		ResponseMessage respMsg = response.readEntity(ResponseMessage.class);
		assertEquals(200, response.getStatus());
		assertEquals("EVENT_DELETION_SUCCESSFUL", respMsg.getMessage());
		assertEquals("The event was successfully deleted from the model", respMsg.getDescription());
	}
}
